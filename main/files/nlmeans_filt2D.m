function im_rec=nlmeans_filt2D(xn,sigmas,ksize,ssize,noise_std)
	...
	[M,N]=size(xn);
	xm = padarray(xn, [half_ssize half_ssize], 'symmetric', 'both');
...
% % %             	sq_dist=sum(sum((dist.^2)))/(ksize^2);
					euclid_dist=sum((xtemp-search_win(kr:kr+ksize-1,kc:kc+ksize-1)).^2, "all");
					wt_dist=times(gauss_win,euclid_dist);
			...
			sum_pix=sum(search_win(half_ksize+1:ssize-half_ksize,half_ksize+1:ssize-half_ksize), "all")*weightn;
